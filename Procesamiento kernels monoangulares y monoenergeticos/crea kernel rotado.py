...
delta_alpha = np.pi/180  #para que nc_rotacion sea igual a 4 grados
delta_beta = np.pi/180  #10 grados

alpha = 45 * np.pi/180  #40 grados
beta  = 0 * np.pi/180
...
kernel_rotado = ker2.RotaKernel(kernel,ang_sol,alpha,beta,ker2.Theta_prima_func,rAxis[:NR],delta_theta,delta_phi)

np.savez("kernel_rotado_prueba.npz", kernel = kernel_rotado, rAxis = rAxis, thetaAxis = thetaAxis)

archivo = np.load("kernel_rotado_prueba.npz")

kernel_rotado = archivo['kernel']

# volume, ang_sol = Integral_vol_del_kernel(kernel_rotado,rAxis[:NR],thetaAxis)
...
ker2.plotPolarContour_full(kernel_rotado, rAxis[:NR], thetaAxis*180/np.pi, 4)